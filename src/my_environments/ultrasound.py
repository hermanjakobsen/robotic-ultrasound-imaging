...
class Ultrasound(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        self.soft_torso = SoftTorsoObject(name='torso')     #, damping=damping[rand_damp_idx], stiffness=stiffness[rand_stiff_idx])

        # Create placement initializer
        if self.placement_initializer is not None:
            self.placement_initializer.reset()
            self.placement_initializer.add_objects(self.soft_torso)
        else:
            self.placement_initializer = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=[self.soft_torso],
                x_range=[-0.10, 0.10],
                y_range=[-0.10, 0.10],
                rotation=None,
