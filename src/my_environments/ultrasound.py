...
class Ultrasound(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self._check_probe_contact_with_torso():
            ...
            # force reward
            force_error = self.force_error_mul * (np.power(total_force_ee - self.goal_contact_force, 2))
            self.force_reward = np.exp(-1 * force_error)
    ...
    def get_contacts_objects(self, model):
        """
    ...
    def _check_probe_contact_with_torso(self):
        ...
        """     
        gripper_contacts = self.get_contacts_objects(self.robots[0].gripper)

        # check contact with torso geoms based on autogenerated names
        for contact in gripper_contacts:
            g1, g2 = self.sim.model.geom_id2name(contact.geom1), self.sim.model.geom_id2name(contact.geom2) 
            match1 = re.search("[G]\d+[_]\d+[_]\d+$", g1)
            match2 = re.search("[G]\d+[_]\d+[_]\d+$", g2)
